...
class APDFT_perturbator(lib.StreamObject):
    ...
    def APDFT3(self, pvec):
        pvec = np.asarray(pvec)
        return self.APDFT2(pvec) + 1 / 6 * np.einsum(
            "ijk,i,j,k", self.cubic_hessian, pvec, pvec, pvec
        )
    ...
    def target_mol(self, pvec):
        ...
        tmol = gto.M(
            ...
            charge=self.mol.charge + sum(pvec),
        )
        return tmol
    ...
    def ap_bsec(self, pvec):
        ...
        if len(ral) != len(tal):
            print(
                ral,
                tal,
                "reference and target lengths do not match!",
                sys.exc_info()[0],
            )
            raise
        bsecorr = 0
        for i in range(len(ral)):
            bsecorr += abse_atom(
                ral[i], tal[i], self.mf.__class__, self.xcf, bs=self.mol.basis
            )
        return bsecorr
