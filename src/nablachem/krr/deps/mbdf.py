...
def angular_integrals(
    size, threeb, alength=158, a=2, grid1=None, grid2=None, angular_scaling=2.4
):
    """
        ...
        for j in range(size):
            ...
            for k in range(size):
                ...
                if threeb[j][k][-1] != 0:
                    ...
                    f2 += (
                        charge * h1 * exponent * (1 + (num2 * angle1 * angle2 * angle3))
                    ) / (atm**angular_scaling)
...
def mbdf_local(
    ...
    if size > 2:
        for i in range(size):
            ...
            twob, threeb = generate_data(
                size, charges[i], coods[i], charges, coods, cutoff_r
            )

            mat[i][:4] = radial_integrals(size, rlength, twob, step_r)

            mat[i][4:] = angular_integrals(
                size,
                threeb,
                alength,
                grid1=grid1,
    ...
    if size > 2:
        ...
        for key in keys:
            ...
            if num != 0:
                ...
                for j in range(num):
                    twob, threeb = generate_data(
                        size, key, elements[key][0][j], charges, coods, cutoff_r
                    )

                    bags[j][:4] = radial_integrals(size, rlength, twob, step_r)

                    bags[j][4:] = angular_integrals(
                        size,
                        threeb,
                        alength,
                        grid1=grid1,
...
def generate_mbdf(
    ...
    coords, cutoff_r = [a2b * _ for _ in coords], a2b * cutoff_r
    ...
    if local:
        if progress_bar == True:
            ...
            mbdf = Parallel(n_jobs=n_jobs)(
                ...
                for charge, cood in zip(charges, coords)
            )
...
def density_estimate(
    reps, nuclear_charges, keys, bin, bandwidth, kernel="gaussian", scaling="root"
):
    """
    ...
    if kernel == "gaussian":
        for i in range(len(nuclear_charges)):
            ...
            for j, k in enumerate(keys):
                ...
                if len(ii) != 0:
                    big_rep[i, j * size : (j + 1) * size] = wKDE(
                        reps[i][ii] / k, bin, bandwidth, kernel, scaling
                    )
...
def generate_df(mbdf, nuclear_charges, bw=0.07, binsize=0.2, kernel="gaussian"):
    ...
    for i in range(fs):
        kde[:, i * gridsize : (i + 1) * gridsize] = density_estimate(
            reps[i], nuclear_charges, keys, bin, bw, kernel
        )
