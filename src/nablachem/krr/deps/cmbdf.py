    ...
    for i in range(size):
        ...
        for j in range(i + 1, size):
            ...
            if rij_norm != 0 and rij_norm < cutoff_r:
                ...
                fcutij, gfcut = 0.5 * (np.cos(arg) + 1), (-np.pi * np.sin(arg)) / (
                    2 * cutoff_r
                )
                ind = rij_norm / rstep
                ac = np.sqrt(z * z2)
                ...
                for k in range(j + 1, size):
                    ...
                    if rik_norm != 0 and rik_norm < cutoff_r:
                        ...
                        fcutik, fcutjk = fcut(rik_norm, cutoff_r), fcut(
                            rkj_norm, cutoff_r
                        )
                        ...
                        cos1 = np.minimum(
                            1.0,
                            np.maximum(np.dot(rij, rik) / (rij_norm * rik_norm), -1.0),
                        )
                        cos2 = np.minimum(
                            1.0,
                            np.maximum(
                                np.dot(-rij, -rkj) / (rij_norm * rkj_norm), -1.0
                            ),
                        )
                        cos3 = np.minimum(
                            1.0,
                            np.maximum(np.dot(rkj, -rik) / (rkj_norm * rik_norm), -1.0),
                        )
            ...
            if j != i:
                ...
                for k in range(size):
                    if k != j and k != i:
                        ...
                        ac, pref, atm, atm1, ind1, sin1, cos1, cos2, cos3 = (
                            threeb_temps[i][j][k]
                        )
                        if ac != 0:
                            grad_distjk, grad_distik = (
                                twob_temps[j][k][5:],
                                twob_temps[i][k][5:],
                            )
                            ind1 = int(ind1)
                            ...
                            gradfcuti = -temp + (
                                gfcutik * grad_distik * fcutij * fcutjk
                            )
            ...
            if rij_norm != 0 and rij_norm < cutoff_r:
                ...
                for k in range(j + 1, size):
                    ...
                    if rik_norm != 0 and rik_norm < cutoff_r:
                        ...
                        cos2 = np.minimum(
                            1.0,
                            np.maximum(np.dot(rij, rkj) / (rij_norm * rkj_norm), -1.0),
                        )
                        cos3 = np.minimum(
                            1.0,
                            np.maximum(np.dot(-rkj, rik) / (rkj_norm * rik_norm), -1.0),
                        ...
                        for i1 in range(m2):
                            for i2 in range(n2):
                                if i2 == 0:
                                    conv1 = (
                                        pref * aconvs[i1][i2][ind1] * cos2 * cos3
                                    ) / atm
                                    conv2 = (
                                        pref * aconvs[i1][i2][ind2] * cos1 * cos3
                                    ) / atm
                                    conv3 = (
                                        pref * aconvs[i1][i2][ind3] * cos2 * cos1
                                    ) / atm
                                else:
                                    conv1 = (pref * aconvs[i1][i2][ind1]) / atm
                                    conv2 = (pref * aconvs[i1][i2][ind2]) / atm
...
def get_cmbdf(charges, coods, convs, pad=None, rcut=10.0, n_atm=1.0, gradients=False):
    ...
    if gradients:
        ...
        twob, twob_grad, threeb, threeb_grad = generate_data_with_gradients(
            size, charges, coods, rconvs, aconvs, rcut, n_atm
        )
...
def get_cmbdf_global(
    charges, coods, asize, rep_size, keys, convs, rcut=10.0, n_atm=1.0
):
    """
...
def get_convolutions(
    ...
    fms = [
        ...
        *[
            gaussian
            * hermite_polynomial(rgrid2, i, np.sqrt(a1))
            * ((-np.sqrt(a1)) ** m)
    ...
    fms = [
        ...
        *[
            gaussian
            * hermite_polynomial(agrid2, i, np.sqrt(a2))
            * ((-np.sqrt(a2)) ** m)
            for i in range(1, m + 1)
        ],
    ]
...
def generate_mbdf(
    ...
    for i in range(len(nuclear_charges)):
        ...
        assert q.shape[0] == r.shape[0], (
            "charges and coordinates array length mis-match for molecule at index"
            + str(i)
        )
    ...
    if type(convs) == str:
        convs = get_convolutions(
            rstep, rcut, alpha_list, n_list, order, a1, a2, astep, nAs, gradients
        )
    ...
    if local:
        if gradients:
            if progress_bar:
                ...
                mbdf = Parallel(n_jobs=n_jobs)(
                    delayed(get_cmbdf)(
                        charge, cood, convs, pad, rcut, n_atm, gradients=True
                    )
            ...
            if progress_bar:
                ...
                reps = Parallel(n_jobs=n_jobs)(
                    delayed(get_cmbdf)(
                        charge, cood, convs, pad, rcut, n_atm, gradients=False
                    )
                    for charge, cood in tqdm(list(zip(charges, coords)))
                )
        ...
        if progress_bar:
            ...
            reps = Parallel(n_jobs=n_jobs)(
                delayed(get_cmbdf_global)(
                    charge, cood, asize, rep_size, keys, convs, rcut, n_atm
                )
                for charge, cood in tqdm(list(zip(charges, coords)))
            )
