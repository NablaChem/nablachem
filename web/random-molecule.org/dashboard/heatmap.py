...
def _interpolate(experiment_data, ds, rep, prop, selected_kernel, slider_val, available_train_sizes, min_train, max_train, metric):
    if slider_val in available_train_sizes:
        entry = next((d for d in experiment_data if d.get("rep") == rep and d.get("dataset") == ds and d.get("prop") == prop and d.get("kernel") == selected_kernel and d.get("train_size") == slider_val), None)
    ...
    entry_lower = next((d for d in experiment_data if d.get("rep") == rep and d.get("dataset") == ds and d.get("prop") == prop and d.get("kernel") == selected_kernel and d.get("train_size") == lower_ts), None)
    entry_upper = next((d for d in experiment_data if d.get("rep") == rep and d.get("dataset") == ds and d.get("prop") == prop and d.get("kernel") == selected_kernel and d.get("train_size") == upper_ts), None)
    
    val_lower = entry_lower.get(metric) if entry_lower else None
    val_upper = entry_upper.get(metric) if entry_upper else None
...
def precalculate_frames(experiment_data, prop, selected_kernel, reps, datasets, min_train, max_train, available_train_sizes):
    ...
    # 1. Base steps with a stride of 8
    base_steps = list(range(min_train, max_train + 1, 8))
    
    # 2. Merge with exact real points to ensure snapping matches real data perfectly
    slider_steps = sorted(list(set(base_steps + available_train_sizes)))
...
def render_heatmap(experiment_data):
    with st.container(border=True):
        ...
        for prop in selected_props:
            ...
            available_train_sizes = sorted(list(set(
                d.get("train_size") for d in experiment_data 
                if d.get("prop") == prop and d.get("train_size") is not None
            )))
            ...
            # Fix 4: Configure Native Plotly Slider — display label text ONLY for true datapoints
            steps = []
            for val in slider_steps_list:
                label_text = str(val) if val in available_train_sizes else ""
                step = dict(
                    method="animate",
                    args=[
                        [str(val)],
